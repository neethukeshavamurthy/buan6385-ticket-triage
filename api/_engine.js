...
const EMPLOYEE_DIRECTORY = {
  // Sample employees used by the test buttons on the admin page
  ...
  // Team members and professor, so their UT Dallas emails are verified employees too
  "saanvi.dusa@utdallas.edu":          { employee_id: "E2001", name: "Saanvi Dusa",           department: "Operations",       tier: "Standard",  active: true, timezone: "America/Chicago" },
  "alicia.penuelas@utdallas.edu":      { employee_id: "E2002", name: "Alicia Penuelas",       department: "Finance",          tier: "Standard",  active: true, timezone: "America/Chicago" },
  "neethu.keshavamurthy@utdallas.edu": { employee_id: "E2003", name: "Neethu Keshava Murthy", department: "Risk",             tier: "Standard",  active: true, timezone: "America/Chicago" },
  "gxs150130@utdallas.edu":            { employee_id: "E2004", name: "Professor",             department: "Academic Affairs", tier: "Executive", active: true, timezone: "America/Chicago" },
};

// Knowledge base used by the "Get KB Match Score" step
const KB_ARTICLES = [
  { id: "KB-101", title: "Reset your password or unlock your account", url: "https://kb.support.utdallas.internal/KB-101",
    keywords: ["password", "reset", "unlock", "locked", "forgot", "login", "sign"] },
  { id: "KB-204", title: "Connect to the office Wi-Fi and VPN", url: "https://kb.support.utdallas.internal/KB-204",
    keywords: ["wifi", "wi-fi", "vpn", "connect", "network", "internet", "globalprotect"] },
  { id: "KB-310", title: "Set up a printer on your laptop", url: "https://kb.support.utdallas.internal/KB-310",
    keywords: ["printer", "print", "printing", "scanner", "setup", "add"] },
  { id: "KB-415", title: "Request PTO and check your leave balance", url: "https://kb.support.utdallas.internal/KB-415",
    keywords: ["pto", "leave", "vacation", "balance", "holiday", "time off", "request"] },
  { id: "KB-520", title: "Book a meeting room", url: "https://kb.support.utdallas.internal/KB-520",
...
function validateDataContract(input) {
  ...
  // Optional phone (spec addition): 10-digit US number. Missing = no SMS, not a failure.
  const digits = String(input.phone || "").replace(/\D/g, "").replace(/^1(?=\d{10}$)/, "");
  if (digits.length === 10) ticket.phone = digits;
  else if (digits.length) flags.push("phone number is not a 10-digit US number: SMS updates skipped");
  ...
  ticket.urgency_selected = ticket.urgency;

  // Urgency can also be inferred from the text. The inferred value wins only if
  // it is HIGHER, so we never silently downgrade what an employee flagged.
  const inferred = inferUrgency(`${ticket.subject} ${ticket.ticket_body}`);
  if (inferred && URGENCIES.indexOf(inferred.level) > URGENCIES.indexOf(ticket.urgency)) {
    flags.push(`urgency raised from ${ticket.urgency} to ${inferred.level} (${inferred.why})`);
    ticket.urgency = inferred.level;
  }
...
function inferUrgency(text) {
  const t = lower(text);
  if (["breach", "hacked", "phishing", "malware", "ransomware"].some((w) => t.includes(w))) return { level: "Critical", why: "possible security incident" };
  if (["locked out", "can't log in", "cannot log in", "outage", "is down", "system crash"].some((w) => t.includes(w))) return { level: "High", why: "access or outage language" };
  return null;
}
...
// Plain-language status labels. The same words are used on the employee page,
// in emails, in SMS and on the dashboard, so they never drift apart.
const TEAM_BY_QUEUE = {
  "People Ops queue": "People Operations", "Desktop Support queue": "Desktop Support", "Identity & Access queue": "Identity & Access",
  "Systems Engineering queue": "Systems Engineering", "Facilities queue": "Facilities", "General triage queue": "Support",
};
function statusCopy(status, info = {}) {
  const team = TEAM_BY_QUEUE[String(info.queue || "").replace(" (priority)", "")] || "Support";
  switch (status) {
    case "Auto-Closed": return { label: "Resolved with a help article",
      next: `This help article should fix it: "${info.kb_title}" (${info.kb_url}). If it doesn't, press "This didn't fix it" or reply, and the ticket reopens.` };
    case "Routed": return { label: `With the ${team} team`, next: `A specialist on the ${team} team is reviewing this. You'll get an email when there's an update.` };
    case "Escalated": return { label: "With a support lead", next: "A support lead is reviewing this now because it needs a person's judgement. You'll hear back by email." };
    case "Flagged for Human Triage": return { label: "Being reviewed by support staff", next: "A member of the support team is reading your ticket to decide who should handle it." };
    case "Hold (Unverified Requester)": return { label: "On hold: email not verified",
      next: `We couldn't match ${info.email || "that email"} to an active employee record. The ticket is on hold until we confirm who you are.` };
    case "Reopened": return { label: "Reopened", next: "We reopened this ticket. A specialist will follow up by email." };
    default: return { label: status, next: "" };
  }
}

// Every message on a ticket shares one email_thread_id and is append-only.
function makeMessage(threadId, direction, channel, subject, body) {
  return { message_id: "msg_" + Date.now().toString(36) + Math.random().toString(36).slice(2, 7), thread_id: threadId,
    at: new Date().toISOString(), direction, channel, subject, body };
}
...
async function runTriage(input, seenKeys) {
  ...
  const threadId = "thr_" + Date.now().toString(36) + Math.random().toString(36).slice(2, 6);
  const finish = (outcome, extra = {}) => {
    warmCache.add(dedupeKey(ticket));
    const copy = statusCopy(outcome, { queue: extra.queue, kb_title: extra.kb && extra.kb.article && extra.kb.article.title,
      kb_url: extra.kb && extra.kb.article && extra.kb.article.url, email: ticket.requester_email });
    const messages = [];
    // Irreversible messages fire only now, once the decision is final, and never
    // to an unverified requester.
    if (outcome !== "Hold (Unverified Requester)") {
      const subj = `[${ticket.ticket_id}] ${ticket.subject || "Your support ticket"}`;
      messages.push(makeMessage(threadId, "out", "email", subj, `We received your ticket ${ticket.ticket_id} and we're on it.`));
      messages.push(makeMessage(threadId, "out", "email", subj, `Status: ${copy.label}. ${copy.next}`));
      if (ticket.phone) messages.push(makeMessage(threadId, "out", "sms", "", `UTD Support ${ticket.ticket_id}: ${copy.label}. Details were sent to your email.`));
      actions.push({ action: `Send confirmation + next-steps email${ticket.phone ? " and SMS" : ""}`, target: ticket.phone ? "Email + SMS" : "Email", reversible: false, owner: "Support lead" });
      step("Send Confirmation + Next Steps (Action)", "4 Actions", "ACTION", ticket.phone ? "email + SMS sent" : "email sent",
        `Only after the status was final. Thread ${threadId}. Cannot be unsent.`);
    } else {
      step("Send Confirmation + Next Steps (Action)", "4 Actions", "ACTION", "not sent", "Requester is not verified, so nothing is emailed");
    }
    return buildTraceRow({ ticket, outcome, path, actions, flags, startedAt, channel, email_thread_id: threadId,
      status_label: copy.label, next_steps: copy.next, messages, ...extra });
  };
  ...
  if (dup.seen) return buildTraceRow({ ticket, outcome: "Dropped (Duplicate)", path, actions, flags, startedAt, channel });
  ...
  if (!who.verified) {
    ...
    return finish("Hold (Unverified Requester)", { requester: who.record && who.record.active ? who.record : null });
  }
...
function buildTraceRow({ ticket, outcome, path, actions, flags, startedAt, channel, email_thread_id, status_label, next_steps, messages, requester, classification, tier, kb, queue, sentiment, escalation_reason }) {
  ...
  return {
    ...
    urgency_selected: ticket.urgency_selected || ticket.urgency,
    email_thread_id: email_thread_id || null,
    status: outcome,
    status_label: status_label || outcome,
    next_steps: next_steps || "",
    trail: messages || [],
    subject: ticket.subject,
    urgency: ticket.urgency,
    category: classification ? classification.category : "General",
    ...
    kb_title: kb && kb.article ? kb.article.title : null,
...
module.exports = { runTriage, parseEmail, statusCopy, makeMessage, CATEGORIES, URGENCIES, EMPLOYEE_DIRECTORY, KB_ARTICLES };
