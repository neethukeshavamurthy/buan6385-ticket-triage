...
const { runTriage, parseEmail } = require("./_engine");
...
module.exports = async (req, res) => {
  ...
  try {
    ...
    body = body || {};
    const { seen_keys, raw_email, ...formTicket } = body;
    // Two ways in: the ticket form, or the backup email intake (Red Team fix).
    const ticket = raw_email ? parseEmail(raw_email) : formTicket;
    ...
    const n8n = trace.outcome === "Dropped (Duplicate)"
      ...
      : await forwardToN8n({
          ...
          source: trace.channel === "email" ? "vercel-app-email" : "vercel-app",
        });
